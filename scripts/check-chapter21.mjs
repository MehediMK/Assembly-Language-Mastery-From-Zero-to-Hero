...
import { CHAPTERS_LEVEL_4 } from '../src/data/chaptersLevel4.ts';

const manuscript = readFileSync(new URL('../Assembly_Language_Mastery_From_Zero_to_Hero.md', import.meta.url), 'utf8');
const source = manuscript.slice(manuscript.indexOf('# Chapter 21:'), manuscript.indexOf('# Chapter 22:'));
const chapter = CHAPTERS_LEVEL_4[3];
...
assert.equal(chapter.sections.length, 39);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'optimization_techniques');
...
console.log(`Chapter 21 coverage passed: ${checked} source lines/table cells; 39 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 21.1 '), source.indexOf('## 21.12 '));
