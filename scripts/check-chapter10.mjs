...
import { CHAPTERS_LEVEL_2 } from '../src/data/chaptersLevel2.ts';

const manuscript = readFileSync(new URL('../Assembly_Language_Mastery_From_Zero_to_Hero.md', import.meta.url), 'utf8');
const source = manuscript.slice(manuscript.indexOf('# Chapter 10:'), manuscript.indexOf('# Chapter 11:'));
const chapter = CHAPTERS_LEVEL_2[4];
...
assert.equal(chapter.sections.length, 19);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'procedures_stack');
...
console.log(`Chapter 10 coverage passed: ${checked} source lines/table cells; 19 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 10.1 '), source.indexOf('## 10.8 '));
