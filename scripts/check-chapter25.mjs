...
import { CHAPTERS_LEVEL_5 } from '../src/data/chaptersLevel5.ts';

const manuscript = readFileSync(new URL('../Assembly_Language_Mastery_From_Zero_to_Hero.md', import.meta.url), 'utf8');
const source = manuscript.slice(manuscript.indexOf('# Chapter 25:'), manuscript.indexOf('# Chapter 26:'));
const chapter = CHAPTERS_LEVEL_5[2];
...
assert.equal(chapter.sections.length, 26);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'stack_frames_prologues');
...
console.log(`Chapter 25 coverage passed: ${checked} source lines/table cells; 26 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 25.1 '), source.indexOf('## 25.10 '));
