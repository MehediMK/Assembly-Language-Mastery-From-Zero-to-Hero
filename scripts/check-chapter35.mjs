...
import { CHAPTERS_LEVEL_7 } from '../src/data/chaptersLevel7.ts';

const manuscript = readFileSync(new URL('../Assembly_Language_Mastery_From_Zero_to_Hero.md', import.meta.url), 'utf8');
const source = manuscript.slice(manuscript.indexOf('# Chapter 35:'), manuscript.indexOf('# Chapter 36:'));
const chapter = CHAPTERS_LEVEL_7[0];
...
assert.equal(chapter.sections.length, 14);
assert.equal(chapter.exercises.length, 9);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'embedded_mcu');
...
console.log(`Chapter 35 coverage passed: ${checked} source lines/table cells; 13 source sections plus retained additions, 9 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 35.1 '), source.indexOf('## 35.8 '));
const headings = [...body.matchAll(/^#{2,3} (.+)$/gm)].map(match => match[1].replace(/`/g, ''));
assert.deepEqual(chapter.sections.filter(section => !section.id.includes('-supplement')).map(section => section.title), headings, 'Keep all source section headings in order');
