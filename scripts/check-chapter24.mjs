...
const source = manuscript.slice(manuscript.indexOf('# Chapter 24:'), manuscript.indexOf('# Chapter 25:'));
const chapter = CHAPTERS_LEVEL_5[1];
...
assert.equal(chapter.sections.length, 31);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'disassembly_analysis');
...
console.log(`Chapter 24 coverage passed: ${checked} source lines/table cells; 31 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 24.1 '), source.indexOf('## 24.10 '));
