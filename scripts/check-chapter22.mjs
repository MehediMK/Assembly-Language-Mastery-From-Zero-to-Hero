...
const source = manuscript.slice(manuscript.indexOf('# Chapter 22:'), manuscript.indexOf('# Chapter 23:'));
const chapter = CHAPTERS_LEVEL_4[4];
...
assert.equal(chapter.sections.length, 25);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'atomic_concurrency');
...
console.log(`Chapter 22 coverage passed: ${checked} source lines/table cells; 25 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 22.1 '), source.indexOf('## 22.9 '));
