...
import { CHAPTERS_LEVEL_6 } from '../src/data/chaptersLevel6.ts';

const manuscript = readFileSync(new URL('../Assembly_Language_Mastery_From_Zero_to_Hero.md', import.meta.url), 'utf8');
const source = manuscript.slice(manuscript.indexOf('# Chapter 30:'), manuscript.indexOf('# Chapter 31:'));
const chapter = CHAPTERS_LEVEL_6[2];
...
assert.equal(chapter.sections.length, 18);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'project_sorting_utils');
...
console.log(`Chapter 30 coverage passed: ${checked} source lines/table cells; 18 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 30.1 '), source.indexOf('## 30.8 '));
