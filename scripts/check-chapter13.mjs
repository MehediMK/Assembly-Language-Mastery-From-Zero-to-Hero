...
const source = manuscript.slice(manuscript.indexOf('# Chapter 13:'), manuscript.indexOf('# Chapter 14:'));
const chapter = CHAPTERS_LEVEL_3[1];
...
assert.equal(chapter.sections.length, 28);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'structures_memory');
...
console.log(`Chapter 13 coverage passed: ${checked} source lines/table cells; 28 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 13.1 '), source.indexOf('## 13.9 '));
