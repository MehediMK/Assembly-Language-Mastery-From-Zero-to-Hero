...
const source = manuscript.slice(manuscript.indexOf('## Chapter 2:'), manuscript.indexOf('# Chapter 3:'));
const chapter = CHAPTERS_LEVEL_1[1];
...
assert.equal(chapter.sections.length, 13);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'data_representation');
...
console.log(`Chapter 2 coverage passed: ${checked} source lines/table cells; 13 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 2.1 '), source.indexOf('## 2.8 '));
const headings = [...body.matchAll(/^#{2,3} (.+)$/gm)].map(match => match[1]);
