...
const source = manuscript.slice(manuscript.indexOf('# Chapter 26:'), manuscript.indexOf('# Chapter 27:'));
const chapter = CHAPTERS_LEVEL_5[3];
...
assert.equal(chapter.sections.length, 29);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'reverse_engineering');
...
console.log(`Chapter 26 coverage passed: ${checked} source lines/table cells; 29 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 26.1 '), source.indexOf('## 26.10 '));
