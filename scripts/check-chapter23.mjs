...
const source = manuscript.slice(manuscript.indexOf('# Chapter 23:'), manuscript.indexOf('# Chapter 24:'));
const chapter = CHAPTERS_LEVEL_5[0];
...
assert.equal(chapter.sections.length, 20);
assert.equal(chapter.exercises.length, 5);
assert.equal(chapter.practiceQuestions.length, 10);
assert.equal(chapter.diagramType, 'executable_elf_pe');
...
console.log(`Chapter 23 coverage passed: ${checked} source lines/table cells; 20 native sections, 5 interactive exercises, 10 answered questions.`);

const body = source.slice(source.indexOf('## 23.1 '), source.indexOf('## 23.7 '));
